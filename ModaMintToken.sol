...
contract ModaMintToken is IERC20, Ownable {
    ...
    // ===== 开盘控制（新增）=====
    uint256 public scheduledTradingTime;   // 定时开盘时间戳，0=未设置
    bool public autoActivateOnFill = true; // 满额是否自动开盘（默认开）
    ...
    // ===== Mint 底池比例（新增）=====
    // mintLiquidityBps: mint 时多少比例的 BNB 加入底池，其余发给部署者
    // 默认 10000 = 100%，即全部加入底池（平开）
    uint256 public mintLiquidityBps = 10000;
    ...
    // 新增事件
    event ScheduledTradingSet(uint256 timestamp);
    event TradingAutoActivated(uint256 timestamp);
    event MintLiquidityBpsChanged(uint256 oldBps, uint256 newBps);
    event MintOwnerBNBSent(address indexed owner, uint256 amount);
    ...
    function _transfer(address from, address to, uint256 amount) internal {
        ...
        if (isDexTransfer && !tradingActive) {
            // 检查是否到了定时开盘时间
            if (scheduledTradingTime != 0 && block.timestamp >= scheduledTradingTime) {
                tradingActive = true;
                emit TradingAutoActivated(block.timestamp);
            } else {
    ...
    /// @notice 设置定时开盘时间（Unix 时间戳）
    /// @param _timestamp 开盘时间戳，设为 0 则取消定时开盘
    function setScheduledTradingTime(uint256 _timestamp) external onlyOwner {
        // 允许设置未来时间，也允许设置 0 取消
        // 如果设置的时间已经过了，下次交易时会自动开盘
        scheduledTradingTime = _timestamp;
        emit ScheduledTradingSet(_timestamp);
    }

    /// @notice 查看是否已达到定时开盘时间
    function canScheduledTrading() public view returns (bool) {
        return scheduledTradingTime != 0 && block.timestamp >= scheduledTradingTime;
    }

    /// @notice 手动触发定时开盘检查（任何人都可以调用，节省 owner gas）
    /// @dev 达到设定时间后调用此函数开盘
    function checkAndActivateTrading() external {
        require(!tradingActive, "Already active");
        require(scheduledTradingTime != 0, "No scheduled time");
        require(block.timestamp >= scheduledTradingTime, "Too early");

        tradingActive = true;
        emit TradingAutoActivated(block.timestamp);
    }

    /// @notice 手动开盘（owner 随时可以强制开盘）
    ...
    /// @notice 设置满额后是否自动开盘（默认 true）
    function setAutoActivateOnFill(bool _v) external onlyOwner {
        autoActivateOnFill = _v;
    }
    ...
    /// @notice 设置 mint 时加入底池的 BNB 比例
    /// @param _bps 比例，单位 bps，10000 = 100%，5000 = 50%
    /// @dev 剩余 BNB 会发给部署者钱包
    function setMintLiquidityBps(uint256 _bps) external onlyOwner {
        require(_bps <= 10000, "Max 10000");
        // 允许设为 0，但那样就不会加底池了，发出去也行，这里不做限制
        emit MintLiquidityBpsChanged(mintLiquidityBps, _bps);
        mintLiquidityBps = _bps;
    }
    ...
    function setMintPrice(uint256 costBNB_, uint256 fillBNB_) external onlyOwner {
        ...
        fillAmountBNB = fillBNB_;
        // 修复：使用 presaleTokenPct_ 参数，但构造函数参数里没有传进来
        // 需要存储 presaleTokenPct_ 或在参数里加回来
        // 临时方案：从当前 tokensPerMint 反推不合适，改为由调用者通过 setTokensPerMint 单独设置
        // 这里只更新 cost 和 fill，tokensPerMint 需要单独设置
    }

    /// @notice 单独设置 tokensPerMint（在 setMintPrice 后调用）
    function setTokensPerMint(uint256 _tokensPerMint) external onlyOwner {
        require(_tokensPerMint > 0, "Zero");
        tokensPerMint = _tokensPerMint;
    }

    /// @notice 一键设置 mint 价格 + tokensPerMint（推荐）
    function setMintConfig(uint256 costBNB_, uint256 fillBNB_, uint256 presaleTokenPct_) external onlyOwner {
        require(costBNB_ > 0, "Cost > 0");
        require(fillBNB_ >= costBNB_, "Fill >= cost");
        require(presaleTokenPct_ >= 1 && presaleTokenPct_ <= 99, "Pct 1-99");
        mintCostBNB = costBNB_;
        fillAmountBNB = fillBNB_;
        tokensPerMint = _totalSupply.mul(presaleTokenPct_).div(100).div(fillBNB_.div(costBNB_));
    }
    ...
    /// @dev 新版 mint：每笔 mint 后按 mintLiquidityBps 比例加底池，剩余 BNB 发给部署者
    function mint() public payable {
        ...
        // ===== 按 mintLiquidityBps 比例处理底池 =====
        _addMintLiquidityV2(tokenAmt, msg.value);
    ...
    /// @dev V2：按 mintLiquidityBps 比例加底池，剩余 BNB 发给部署者
    function _addMintLiquidityV2(uint256 tokenAmt, uint256 bnbAmt) internal {
        if (mintLiquidityBps == 0) {
            // 0% 加池，全部 BNB 发给部署者
            if (bnbAmt > 0) {
                (bool ok, ) = owner().call{value: bnbAmt}("");
                if (ok) emit MintOwnerBNBSent(owner(), bnbAmt);
            }
            return;
        }

        // 计算用于加池的 BNB 数量
        uint256 bnbForLP = bnbAmt.mul(mintLiquidityBps).div(10000);
        uint256 bnbForOwner = bnbAmt.sub(bnbForLP);

        // 计算用于加池的代币数量（等比例）
        uint256 tokensForLP = tokenAmt.mul(mintLiquidityBps).div(10000);

        // 加底池
        if (tokensForLP > 0 && bnbForLP > 0) {
            // 检查合约还有没有足够代币来加池
            if (_balances[address(this)] >= tokensForLP) {
                _balances[address(this)] = _balances[address(this)].sub(tokensForLP);
                ...
                try uniswapV2Router.addLiquidityETH{value: bnbForLP}(
                    ...
                    emit MintLiquidityAdded(tokensForLP, bnbForLP);
                } catch {
                    // 加池失败：代币退回合约，BNB 直接发给部署者
                    _balances[address(this)] = _balances[address(this)].add(tokensForLP);
                    bnbForOwner = bnbForOwner.add(bnbForLP);
                }
            } else {
                // 代币不够，全部 BNB 发给部署者
                bnbForOwner = bnbForOwner.add(bnbForLP);
            ...
            // tokensForLP = 0 的情况，bnbForLP 也发给部署者
            bnbForOwner = bnbForOwner.add(bnbForLP);
        }

        // 剩余 BNB 发给部署者
        if (bnbForOwner > 0) {
            (bool ok, ) = owner().call{value: bnbForOwner}("");
            if (ok) emit MintOwnerBNBSent(owner(), bnbForOwner);
        }
    }
